...
| F-08 | Condition evidence span and full raw text toggle | Applied/notMet condition rows render `condition.evidence`; `<details>` renders `rawSpecialCondition`. `excluded` conditions are not rendered as condition cards, so F-08 card evidence does not apply to them; their source wording is still available in the full raw disclosure toggle. | Match with scope note |
| F-08 | `AI 해석` badge on AI-interpreted entries | `ConditionList` renders `AI 해석` for every applied/notMet condition row. This is acceptable because all committed condition rows are AI-extraction outputs reviewed in `docs/records/2026-09-03-condition-extraction.md`; the badge therefore marks the extraction method, not low confidence. | Match |
